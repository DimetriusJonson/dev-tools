...
pub async fn format_xml_handler(RawQuery(query): RawQuery, body: Body) -> impl IntoResponse {
    ...
    let stream = stream! {
        ...
        loop {
            match input_xml_reader.read_event_into_async(&mut read_buffer).await.unwrap() {
                Event::Text(ref e) => {
                    let text_content = input_xml_reader.decoder().decode(e).unwrap();
                    ...
                    if !filtered_lines.is_empty() {
                        ...
                        writer.write_event(Event::Text(BytesText::new(&filtered_text))).unwrap();
                    }
                }
                Event::Comment(e) => {
                    writer.write_event(Event::Comment(e)).unwrap();
                }
                Event::Eof => break,
